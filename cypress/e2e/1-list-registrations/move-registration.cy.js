...
describe('Registrations List', () => {
  ...
  afterEach(() => {
    cy.request('DELETE', `http://localhost:3000/registrations/${fixtures[0].id}`);
  })
  ...
  it('should be able to move from REVIEW column to APPROVED column', () => {
    cy.get('[data-testci="column-REVIEW"]')
      ...
      .find('button').contains('Aprovar').click();

    cy.get('[role="dialog"]')
    .should('exist')
    .find('button').contains('Confirmar').click();

    cy.get('[role="status"]').should('contain', 'Status atualizado com sucesso!');
