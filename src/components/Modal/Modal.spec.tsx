...
describe('Modal component', () => {
    ...
    render(
      <Modal.Root open={isOpen} onOpenChange={onOpenChange}>
        <Modal.Content>
          ...
          <Modal.Description>{modalData.content}</Modal.Description>
          <Modal.Footer>
            <button onClick={() => {isOpen = false; onOpenChange()}}>{modalData.confirm}</button>
          </Modal.Footer>
        </Modal.Content>
      </Modal.Root>
    );
  ...
  it('applies correct styles for overlay and content', () => {
    ...
    expect(content).toHaveStyle('box-shadow: hsl(206 22% 7% / 35%) 0px 10px 38px -10px,hsl(206 22% 7% / 20%) 0px 10px 20px -15px');
